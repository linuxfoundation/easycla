...
describe('To Validate & get health status via API call (V3)', function () {
  //Reference api doc: V3 API health endpoint
  ...
  it('Returns the Health of the application- Triple test for flakiness', function () {
    // Run test 3 times to catch flaky behavior
    cy.wrap([1, 2, 3]).each((iteration) => {
      cy.task('log', `Health test iteration ${iteration}/3`);
      return cy
        .request({
          method: 'GET',
          url: `${claEndpoint}ops/health`,
    ...
    it('Returns errors due to malformed requests for Health APIs', function () {
      ...
      const cases: Array<{
        ...
        },
        {
          ...
          expectedMessageLocal: 'path /v3/ops/health/invalid-path was not found',
          expectedMessageContainsLocal: true,
          expectedStatusRemote: 404,
          expectedMessageRemote: 'path /v3/ops/health/invalid-path was not found',
          expectedMessageContainsRemote: true,
        },
      ];
