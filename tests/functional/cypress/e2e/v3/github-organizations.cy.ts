...
describe('To Validate & test GitHub Organizations APIs via API call (V3)', function () {
  //Reference api doc: V3 API github-organizations endpoints
  ...
  it('Get GitHub Organizations with authentication - Record should return 200 Response', function () {
    const projectSFID = 'a092M00001IV4SfQAL'; // Example SFID

    cy.request({
      method: 'GET',
      url: `${claEndpoint}project/${projectSFID}/github/organizations`,
      timeout: timeout,
  ...
  it('Triple test for flakiness - GitHub Organizations endpoints', function () {
    // Run test 3 times to catch flaky behavior
    const projectSFID = 'a092M00001IV4SfQAL';

    cy.wrap([1, 2, 3]).each((iteration) => {
      cy.task('log', `GitHub Organizations test iteration ${iteration}/3`);
      return cy
        .request({
          method: 'GET',
          url: `${claEndpoint}project/${projectSFID}/github/organizations`,
  ...
  describe('Authentication Required Tests', () => {
    it('Returns 401 for GitHub Organizations APIs when called without token', () => {
      ...
      const exampleOrgName = 'test-org';

      const requests = [
        // GET /project/{projectSFID}/github/organizations (requires auth)
        { method: 'GET', url: `${claEndpoint}project/${exampleProjectSFID}/github/organizations` },

        // POST /project/{projectSFID}/github/organizations (requires auth if it exists)
        {
          method: 'POST',
          url: `${claEndpoint}project/${exampleProjectSFID}/github/organizations`,
          body: { organization_name: exampleOrgName },
        },

        // DELETE /project/{projectSFID}/github/organizations/{orgName} (requires auth if it exists)
        { method: 'DELETE', url: `${claEndpoint}project/${exampleProjectSFID}/github/organizations/${exampleOrgName}` },
      ];
  ...
  describe('Expected failures', () => {
    it('Returns errors due to malformed requests for GitHub Organizations APIs', function () {
      ...
      const cases: Array<{
        ...
        {
          title: 'GET /project/{invalidSFID}/github/organizations (bad request)',
          method: 'GET',
          url: `${claEndpoint}project/${invalidSFID}/github/organizations`,
          needsAuth: true,
          expectedStatusLocal: 400,
          expectedStatusRemote: 400,
        },
        {
          title: 'POST /project/{validSFID}/github/organizations with empty body (bad request)',
          method: 'POST',
          url: `${claEndpoint}project/a092M00001IV4SfQAL/github/organizations`,
          body: {},
          needsAuth: true,
          expectedStatusLocal: 400,
          expectedStatusRemote: 400,
        ...
        {
          title: 'PUT /project/{validSFID}/github/organizations (method not allowed)',
          method: 'PUT',
          url: `${claEndpoint}project/a092M00001IV4SfQAL/github/organizations`,
          body: {},
