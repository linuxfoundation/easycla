...
func (s *service) populateSignURL(ctx context.Context,
	...
	userDetails, err := s.populateUserDetails(ctx, signatureReferenceType, latestSignature, claManagerName, claManagerEmail, sendAsEmail, preferredEmail, allowLFEmail)
	if err != nil {
...
func (s *service) populateUserDetails(ctx context.Context, signatureReferenceType string, latestSignature *signatures.ItemSignature, claManagerName, claManagerEmail string, sendAsEmail bool, preferredEmail string, allowLFEmail bool) (*UserSignDetails, error) {
	f := logrus.Fields{
	...
	if signatureReferenceType == utils.SignatureReferenceTypeCompany {
		...
		if !sendAsEmail {
			...
			if getUserEmail(userModel, preferredEmail, allowLFEmail) != "" {
				userSignDetails.userSignatureEmail = getUserEmail(userModel, preferredEmail, allowLFEmail)
			}
		}
	} else {
