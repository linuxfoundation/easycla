...
// InvalidateProjectRecordWithMetadata mocks base method.
func (m *MockSignatureRepository) InvalidateProjectRecordWithMetadata(ctx context.Context, signatureID, note string, metadata *signatures0.InvalidationMetadata) error {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "InvalidateProjectRecordWithMetadata", ctx, signatureID, note, metadata)
	ret0, _ := ret[0].(error)
	return ret0
}

// InvalidateProjectRecordWithMetadata indicates an expected call of InvalidateProjectRecordWithMetadata.
func (mr *MockSignatureRepositoryMockRecorder) InvalidateProjectRecordWithMetadata(ctx, signatureID, note, metadata interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "InvalidateProjectRecordWithMetadata", reflect.TypeOf((*MockSignatureRepository)(nil).InvalidateProjectRecordWithMetadata), ctx, signatureID, note, metadata)
}
