...
- `cla-backend-go/v2/my_clas/repository.go` — DynamoDB query for a user's ICLA/ECLA records
- `cla-backend-go/v2/my_clas/service_test.go` — unit tests
- `cla-backend-go/v2/user-service/client.go` — one new read helper `ListUserIdentities` (`GET /user-service/v1/users/{sfid}/identities`, mirrors the existing `ListUsersByUsername` pattern)
...
`service.go` / `repository.go`) and is **completely self-contained**: it adds no
behavior changes to any existing endpoint, service, or repository (the only shared-code
touch points are the three registration lines in `cmd/server.go`).
...
1. The user's own EasyCLA user record is fetched by the token username
   (`lf-username-index`). A key is allowed when it matches that record:
   emails against `lf_email` + `user_emails` (case-insensitive), `githubId` against
...
2. Keys not covered there are checked against the **LF-wide identities** connected to
   the account in the platform user-service (loaded lazily, once per request):
   `GET /user-service/v1/users?username={lfid}` for the profile (SFID + all
   non-deleted profile emails) and `GET /user-service/v1/users/{sfid}/identities` for
   connected identities. Usernames are accepted per source (`github`, `gitlab`,
   `gerrit` — a Slack identity never authorizes a GitHub search); identity emails are
   accepted from any source. Numeric GitHub/GitLab IDs cannot be validated through
   user-service (identities carry usernames, not provider IDs), so they validate only
   against the EasyCLA record.
...
| Identity key | Users-table access | Existing service method reused |
|---|---|---|
| `lfUsername` (token) | `lf-username-index` GSI | `users.Service.GetUserByLFUserName` |
| `gerritUsername` | `lf-username-index` GSI (Gerrit = LF SSO identity) | `users.Service.GetUserByLFUserName` |
| `email` | `lf-email-index` GSI | `users.Service.GetUsersByLFEmail` |
| `secondaryEmail` | **table scan** over the `user_emails` set (opt-in only) | `users.Service.GetUsersByEmail` |
| `githubId` | `github-id-index` GSI (N-typed key) | `users.Service.GetUserByGitHubID` |
| `githubUsername` | `github-username-index` GSI | `users.Service.GetUserByGitHubUsername` |
| `gitlabId` | `gitlab-id-index` GSI (N-typed key) | `users.Service.GetUserByGitlabID` |
| `gitlabUsername` | `gitlab-username-index` GSI | `users.Service.GetUserByGitLabUsername` |

"Not found" results are skipped silently; any other lookup error fails the request
(`500`) rather than silently returning a partial history — an incomplete list would
erode user trust (spec: "an incomplete list here erodes trust in every later
milestone").
...
default `email` parameter therefore stays strictly index-backed (`lf_email`), and the
scan runs only for values the caller explicitly passes as `secondaryEmail`. In
practice GitHub/GitLab-derived records (the ones whose `user_emails` would matter
most) are matched precisely by the numeric `githubId`/`gitlabId` keys instead.
...
| `pdfAvailable` | bool | `true` for ICLAs only (ECLAs have no signed document — FR-002); the PDF remains downloadable even for invalidated ICLAs (it is the user's own signed legal record) |
...
- one GSI query for the caller's own EasyCLA record (enforcement) + zero, one or two
  platform user-service HTTP calls (profile + identities — loaded lazily, only when a
  requested key is not already covered by the EasyCLA record, and at most once);
...
No table scans except the explicitly opt-in `secondaryEmail` match (one scan per
provided value — callers should treat it as a slow path). For the read-only Me-lens
page (small per-user datasets) this is well within the v4 Lambda's normal latency
envelope.
...
- **GitLab group approval lists** (`gitlab_org_approval_list`) are not re-evaluated
  live: `UserIsApproved` (the shared gating helper) checks GitLab usernames but group
  membership requires the GitLab org's OAuth token (the MR-gating service does this
  with per-org credentials). An ECLA authorized *solely* via GitLab group membership
  and never invalidated shows `approved=true, valid=false`. Rare in practice; the
  consumer can choose to trust `approved` for display if this matters before a
  follow-up adds the live group check.
- **Secondary emails** (`user_emails`) are matchable only via the opt-in
  `secondaryEmail` parameter, which costs a table scan per value (set attribute, not
  indexable) — the default `email` parameter stays index-backed; numeric GitHub/GitLab
  IDs remain the preferred high-precision keys.
- **Numeric GitHub/GitLab IDs can only be verified against the caller's EasyCLA
  record** — the platform user-service identities carry usernames, not provider IDs.
  An ID not present on the EasyCLA record is skipped even if the matching username
  would have been allowed; callers should pass the username alongside the ID.
