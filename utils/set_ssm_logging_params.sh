...
# STAGE=dev
# SET=1 
# STAGE=dev REGION=us-east-1 SET=1 ./set_ssm_logging_params_dev_py.sh
if [ -z "$STAGE" ]
then
  export STAGE=dev
fi
...
if [ ! -z "$SET" ]
then
  ./utils/set_ssm_value.sh cla-dd-site-dev '?' String
  ./utils/set_ssm_value.sh cla-dd-api-key-secret-arn-dev "$(cat ./DD_API_KEY_SECRET_ARN.secret)" String
  ./utils/set_ssm_value.sh cla-dd-extension-layer-arn-dev '?' String
