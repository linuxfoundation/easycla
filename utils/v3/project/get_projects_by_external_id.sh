#!/bin/bash
# GET /project/external/{projectSFID}
# Getprojectsbyexternalid (authenticated)
# Example: ./get_projects_by_external_id.sh param1
# TOKEN="$(cat ./token.secret)" XACL="$(cat ./x-acl.secret)" ./get_projects_by_external_id.sh <project_sfid>
...
API="${API_URL}/v3/project/external/${project_sfid}"
