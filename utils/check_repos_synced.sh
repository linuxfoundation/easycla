...
then
  ...
  do
    ...
    then
      echo "$repo: WARNING (cannot read origin; not auto-discovered)" >&2
      continue
    fi
    origin="${origin%.git}"
    case "$origin" in
      "https://github.com/linuxfoundation/$repo"|"git@github.com:linuxfoundation/$repo"|"ssh://git@github.com/linuxfoundation/$repo")
        repos="$repos $repo"
        ;;
    esac
  done
