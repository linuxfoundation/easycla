...
def needs_auth(tags, path):
    ...
    public_paths = ['/ops/health', '/ops/version', '/api-docs', '/swagger.json']
...
def generate_script_content(api_info):
    ...
    if param_usage:
        ...
        content.append(f'# API_URL=https://api-gw.dev.platform.linuxfoundation.org/cla-service ./{script_name}')
    ...
    content.append(f'API="${{API_URL}}/v4{api_path}"')
...
def main():
    ...
    swagger_file = os.path.join(project_root, 'cla-backend-go/swagger/cla.v2.compiled.yaml')
