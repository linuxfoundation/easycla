#!/bin/bash
# Get ICLA signatures for a CLA group (authenticated)
# Usage: ./get_cla_group_icla_signatures.sh <cla_group_id> [approved] [signed] [sortOrder]
# Example: ./get_cla_group_icla_signatures.sh d9428888-122b-4b20-8c4a-0c9a1a6f9b8e true true asc
# TOKEN="$(cat ./token.secret)" XACL="$(cat ./x-acl.secret)" ./get_cla_group_icla_signatures.sh <cla_group_id>
...
API="${API_URL}/v4/cla-group/${cla_group_id}/icla/signatures"
if [ ! -z "$QUERY_PARAMS" ]; then
