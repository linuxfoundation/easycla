...
- **Embargo/OFAC attestation**: a mandatory checkbox gates the Sign button. **Verified nuance**: the backend persists `signature_embargo_acked=true` unconditionally in the signing paths (`v2/sign/service.go:483,741,975,1236,1546`) — the actual gate is **client-side in the console**, and `/request-individual-signature` is public, so a direct API caller can skip the checkbox entirely. Reimplementing the checkbox in SS is not sufficient: **M2 must require and validate the attestation server-side** (reject the signing-URL request unless the acknowledgement is present) rather than reproducing the client-only gate. Raise the missing server-side enforcement upstream as a compliance-control gap.
