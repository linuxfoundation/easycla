...
**Record classification** (applied upstream): the query accepts `signature_type` of both `cla` and `ecla` — ICLAs and DocuSign-era ECLAs are stored as `cla`, while ECLAs auto-created from Approved List changes are stored as `ecla`. User-referenced rows are then split on company-ID presence: no company ID ⇒ **ICLA** (has PDF); company ID set ⇒ **ECLA** (no PDF). `ccla` records are corporate data, excluded from M1 (Organization lens, M3). Full field-level detail is in [docs/MY_CLAS_API.md](../../docs/MY_CLAS_API.md).
