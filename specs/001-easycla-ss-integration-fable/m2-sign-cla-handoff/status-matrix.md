...
| **—** (`unknown`) | ECLA only | Coverage could not be confirmed. **Today this is a catch-all**, not purely a transient fault: it covers a genuine lookup/screening failure (retry may help), a missing or lapsed CCLA (E7a — a definitive answer, retry will not help), and GitLab group membership, which is never checkable. Plain text, **not a named pill** | no | Request Removal is retained (it is gated only on "ECLA and not Revoked") |
