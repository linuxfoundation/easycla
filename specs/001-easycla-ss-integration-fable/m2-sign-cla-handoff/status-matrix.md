...
| **Invalidated** | ICLA + ECLA | `signature_approved=false` — the signature itself was invalidated externally | ICLA invalidated in PCC: `Invalidated · <date>` from [#1732](https://github.com/linuxfoundation/lfx-self-serve/issues/1732). Approved List removal and CLA-group deletion write no timestamp, so those records are undated | ICLA: none. ECLA: Request Removal is retained. Contributor remediation is an open legal question — see decisions |
...
| E7a | true | no | **confirmed** absent: the employer holds no approved+signed CCLA for the CLA group | **Needs attention** | `needs_attention` / `no_active_ccla` | ❌ **not built** — ships as `unknown`/"—". Decided 2026-08-24; see decision 5 |
...
- Wire contract: `swagger/common/my-cla.yaml:58-77` — `status` enum (five values, documented as extensible) and `statusReason` enum (`not_on_approval_list`, `unknown`). The decided `no_active_ccla` reason (E7a) is **not in the enum yet**.
