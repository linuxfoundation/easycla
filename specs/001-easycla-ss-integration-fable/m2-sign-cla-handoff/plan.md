...
4. **Status evaluation** (open question 3) — retired upstream: `GET /v4/my-clas` already computes the coverage evaluation per ECLA row; M2 exposes it as a status field instead of collapsing it into `Valid`. **Small SS-side delta remains** (verified 2026-08-11): M1's BFF filters out every non-valid row and derives `status` from the boolean, both in `server/services/cla.service.ts` — see Source Code.
5. **Console empty-project on deep link** (new, found during Spike 1 — not previously tracked). `ClaDashboardComponent` reads the project from browser storage rather than fetching it (`cla-dashboard.component.ts:57,63`), so a cold deep link renders both ICLA and ECLA buttons disabled. Affects FR-002 for **both** agreement types, including the ECLA path previously believed to work proactively as-is. Likely a small `ClaDashboardComponent` fetch. **Confirm with a dev-Console deep-link smoke test before sizing** — this is the one spike conclusion not yet verified against a running system.

If the budget forces a cut: the status column rides the existing evaluation (plus the small BFF change in risk 4); the **GitHub sign entry is not cuttable to "existing endpoints"** — it requires the no-PR ICLA delta (risk 2) — but that delta is now measured and small, so **narrowing to Gerrit-only is no longer a meaningful saving** and is not recommended; ICLA invalidation requires the revision in risk 2a; ECLA invalidation (with FR-008a templates) remains the cleanest deferrable slice. Risk 5 is not deferrable if the sign entry ships at all, since it gates the hand-off landing.
