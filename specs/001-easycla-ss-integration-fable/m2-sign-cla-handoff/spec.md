...
**Invalidation** — primary use cases (per PM, 2026-08-07): *ICLA* — the contributor signed an ICLA when they should have signed an ECLA (contributing on an employer's behalf) and needs to correct it; *ECLA* — the contributor changed employers. Both scenarios end with the user signing the correct/new agreement via the "+ Sign a CLA" entry, so the post-invalidation state SHOULD point there. Simplicity decisions (2026-08-07): invalidation is strictly **per-row** (per signature — an employer change spanning several CLA groups means invalidating each row; no bulk action); ICLA and ECLA are **independent agreements** — no ordering is enforced between correcting flows and no eligibility warnings are added to the modals (copy stays per mockup).
