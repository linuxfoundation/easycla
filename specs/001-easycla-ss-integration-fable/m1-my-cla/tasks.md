...
- [ ] T011 [P] Add SS env config: `CLA_SERVICE_BASE_URL` (lfx-gateway `/cla-service` base) in `apps/lfx-one/.env.example` + server config loading, following how existing service base URLs are declared. Include the flag name from T006 in the LD flag constants if the repo centralizes them.
- [ ] T012 [P] Create SS types in `server/types/cla.types.ts` — upstream response shapes (from T004 fixtures) + `ResolvedClaIdentity { lfUsername, emails[], githubIds[], easyclaUserIds[] }` per data-model.md — and the shared UI interface `MyClaAgreement`/`MyClasResponse`/`PdfUrlResponse` in `packages/shared/src/interfaces/` (exact file per repo convention).
...
- [ ] T016 [US1] Implement `server/services/cla.service.ts`: EasyCLA client (per T002: user api-gw token via the existing exchange, or M2M fallback) with methods `resolveIdentity(session) → ResolvedClaIdentity` (LF username + verified emails from session/auth-service + GitHub IDs from `Auth0Service.getUserIdentities()` parsed per T003; calls `GET /v4/users/by-identity`, falling back to `/v3/users/username/{userName}` until T010 lands), `getUserSignatures(userIds[])` (paginate `pageSize`/`nextKey` per user, merge), `getSignedDocumentUrl(signatureID)`. Pure functions for classification/status/merge so T013 tests pass.
- [ ] T017 [US1] Implement `server/controllers/clas.controller.ts`: session → `resolveIdentity` → aggregate agreements → `MyClasResponse` with `identity.{matchedUserIds, unmatched, githubLinked}`; PDF handler re-verifies the requested `signatureID` is an ICLA in the session's resolved agreement set before calling upstream (authorization boundary per R3), returns `PdfUrlResponse`.
- [ ] T018 [US1] Implement `server/routes/clas.route.ts` (`GET /api/me/clas`, `GET /api/me/clas/:signatureId/pdf-url`), register it in the server route index behind the LD flag, wire standard auth middleware. T015 tests pass.
...
- [ ] T020 [P] [US1] Create `app/modules/my-clas/my-clas.routes.ts` (exported `MY_CLAS_ROUTES`) and register lazy route `me/clas` with `data: { lens: 'me' }` + LD flag guard in `app/app.routes.ts`; add the Me-lens sidebar item behind the same flag in `app/layouts/main-layout/main-layout.component.ts` (mirror the crowdfunding flag usage).
- [ ] T021 [P] [US1] Create `app/modules/my-clas/components/agreement-card/` — per-agreement row: kind badge (ICLA/ECLA), project name, company name (ECLA only), signed date, status label, document version if present, **Download PDF** action only when `pdfAvailable` (calls the pdf-url route on click and opens the returned URL immediately — never prefetched, 15-min TTL).
- [ ] T022 [US1] Create `app/modules/my-clas/my-clas.component.ts|html`: fetch `/api/me/clas`; ICLA and ECLA sections sorted `signedOn` desc; empty state explaining CLAs with docs link (AS3); "need to sign?" link-out to the Contributor Console (env-configured URL, AS4); "history may be incomplete" hint when `identity.unmatched`; upstream-error state with retry (502 handling).
- [ ] T023 [US1] Add the GitHub-link CTA (FR-005a): when `identity.githubLinked=false` (or agreements empty), render "Don't see your CLAs? Link your GitHub account" pointing into SS's existing identity-linking flow (social-connection pattern); on return/relink, re-fetch `/api/me/clas`. Component tests for CTA visibility rules.
