...
**Project display name — gap (verified)**: the user-signatures payload carries `projectID` (the CLA Group ID) but **no project display name**. M1 currently renders the ID as `projectName` (fallback). Resolving the human-readable name needs the R5 lookup (project-service or a name added to the upstream response) — captured for T004 and the by-identity endpoint work; not a blocker for the read-only list but visible in the UI until resolved.
