...
- `cla-backend-go/v2/my_clas/repository.go` — DynamoDB query for a user's ICLA/ECLA records + the single-scan secondary-email lookup
- `cla-backend-go/v2/my_clas/service_test.go` — unit tests
- `cla-backend-go/v2/user-service/client.go` — one new (additive) read helper `ListUserIdentities` (`GET /user-service/v1/users/{sfid}/identities`, paginated, mirrors the existing `ListUsersByUsername` pattern; no existing method changed)
...
2. Keys not covered there are checked against the **LF-wide identities** connected to
   ...
   connected identities (paginated — all pages are fetched, so accounts with more
   than 100 identities are fully covered). Usernames are accepted per source (`github`, `gitlab`,
...
| `secondaryEmail` | **one table scan** over the `user_emails` set for all values (opt-in only) | module repository `GetUsersBySecondaryEmails` (OR'd `contains` filters) |
...
milestone"). The same rule applies to the display lookups: a *missing* CLA group or
company record degrades gracefully (name omitted / ECLA marked invalid), while any
other data-layer error fails the request.
